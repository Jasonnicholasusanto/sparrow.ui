...
function DitheredWaves({
  ...
  // Keep resolution uniform in sync with real framebuffer size
  useEffect(() => {
  ...
  // Global pointer tracking (works even when canvas is behind other DOM)
  useEffect(() => {
    ...
    const updateFromEvent = (e: PointerEvent) => {
      ...
      // Convert client coords - coords relative to canvas
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;

      // Only update if the pointer is over the canvas bounds
      if (x < 0 || y < 0 || x > rect.width || y > rect.height) {
        mouseRef.current.set(-9999, -9999);
        return;
      }

      // Convert CSS pixels: framebuffer pixels (matches your shader's resolution uniform)
      const dpr = gl.getPixelRatio();
      mouseRef.current.set(x * dpr, y * dpr);
    };

    // Capture helps if something in the foreground stops propagation
    window.addEventListener("pointermove", updateFromEvent, { capture: true });

    // Handle touch / pen leaving canvas cleanly
    window.addEventListener("pointerdown", updateFromEvent, { capture: true });

    return () => {
      window.removeEventListener("pointermove", updateFromEvent, {
        capture: true,
      } as any);
      window.removeEventListener("pointerdown", updateFromEvent, {
        capture: true,
      } as any);
    };
